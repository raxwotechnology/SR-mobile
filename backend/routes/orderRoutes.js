...
const { updateOrderAdmin } = require('../controllers/adminController');
...
router.route('/:id')
  ...
  .put(protect, authorize('cashier', 'manager', 'admin'), updateOrderAdmin);
router.route('/:id/status').put(protect, authorize('manager', 'admin'), updateOrderStatus);
router.route('/:id/payhere-hash').post(protect, generatePosPayHereHash);
