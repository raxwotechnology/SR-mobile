...
const InvoiceModal = ({ isOpen, onClose, order, onNewSale }) => {
  ...
  // Auto-print receipt upon checkout completion
  useEffect(() => {
    if (isOpen && order && !hasAutoPrinted) {
  ...
  }, [isOpen, order, hasAutoPrinted]);

  // Keyboard listener for Enter & Escape keys
  useEffect(() => {
    if (!isOpen) return;
